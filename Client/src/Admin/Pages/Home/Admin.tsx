...
const LinkItems: Array<LinkItemProps> = [
     ...
     { name: 'User', icon: FaUserAlt, path: "users" },
     ...
     return (
          <Box transition="3s ease" bg={useColorModeValue('white', 'gray.900')} borderRight="1px" borderRightColor={useColorModeValue('gray.200', 'gray.700')} w={{ base: 'full', md: 60 }} pos="fixed" h="full"{...rest}>
               ...
               {LinkItems.map((link, i) => (
                    <Text as={Link} key={i} to={`/admin/${link.path}`} style={{ textDecoration: 'none' }} _focus={{ boxShadow: 'none' }}>
                         <Flex align="center" p="4" mx="4" borderRadius="lg" role="group" cursor="pointer" _hover={{ bg: 'cyan.400', color: 'white', }}{...rest}>
                              <Icon mr="4" fontSize="16" _groupHover={{ color: 'white', }} as={link.icon} />
                              {link.name}
                         </Flex>
                    </Text>
               ))}
...
const MobileNav = ({ onOpen, ...rest }: MobileProps) => {
     return (
          <Flex ml={{ base: 0, md: 60 }} px={{ base: 4, md: 4 }} bg={useColorModeValue('white', 'gray.900')} height="20" alignItems="center" borderBottomWidth="1px" borderBottomColor={useColorModeValue('gray.200', 'gray.700')} justifyContent={{ base: 'space-between', md: 'flex-end' }}{...rest}>
